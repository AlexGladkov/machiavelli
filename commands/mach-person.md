...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" person "<description>" --json
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" relation confirm <edge_id> --json
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" fact "<person_code>" "связь с ego: <relation>" --json
```
...
→ node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" graph --json
→ node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" relation confirm <edge_id> --json
```
