...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" status --json
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" profile "<person_ref>" --json
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/machiavelli.cjs" advice "<query with optional market context>" --json
```
